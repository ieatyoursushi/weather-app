...
export default async function WeatherData(cityName: string, i: number) {
    if(cityName == '') 
        return;
    const data = await GeoLocation(cityName)
    let longitude: number = data.results[i].longitude;
    let latitude: number = data.results[i].latitude;
